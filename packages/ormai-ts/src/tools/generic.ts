...
const QueryInputSchema = z.object({
  ...
  take: z.number().int().min(1).max(100).default(25).describe('Max rows to return'),
...
const BulkUpdateInputSchema = z.object({
  model: z.string().describe('The model to update'),
  ids: z.array(z.unknown()).min(1).max(100).describe('Primary keys of records to update'),
...
export function createGenericTools(options: GenericToolsOptions): BaseTool<unknown, unknown>[] {
  const { adapter, policy, schema, includeWrite = false } = options;

  const tools: BaseTool<unknown, unknown>[] = [
    new DescribeSchemaTool(schema, policy),
    new QueryTool(adapter, policy, schema),
  ...
  if (includeWrite || policy.writesEnabled) {
    tools.push(
      ...
      new BulkUpdateTool(adapter, policy, schema)
    );
  }
