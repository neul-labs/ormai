...
import type { Budget, ModelPolicy, Policy, RowPolicy } from './models.js';
...
export class PolicyEngine {
  ...
  validateCreate(request: CreateRequest, ctx: RunContext): PolicyDecision {
    ...
    decision.addDecision('Create operation allowed by policy');

    // 3. Check reason requirement
    if (writePolicy.requireReason && !request.reason) {
      throw new ValidationError('A reason is required for this write operation', 'reason');
    }
  ...
  validateUpdate(request: UpdateRequest, ctx: RunContext): PolicyDecision {
    ...
    decision.addDecision('Update operation allowed by policy');
  ...
  validateBulkUpdate(request: BulkUpdateRequest, ctx: RunContext): PolicyDecision {
    ...
    if (!writePolicy.allowBulk) {
      throw new WriteDisabledError('bulk_update', request.model);
    }
    decision.addDecision('Bulk update operation allowed by policy');
    ...
    if (writePolicy.requireReason && !request.reason) {
      throw new ValidationError('A reason is required for this bulk operation', 'reason');
    }
  ...
  private validateAndGetScopeFilters(
    ...
    rowPolicy: RowPolicy,
    ctx: RunContext,
    _existingFilters: readonly FilterClause[] | undefined
  ): FilterClause[] {
    const filters: FilterClause[] = [];

    // Tenant scoping
    if (rowPolicy.tenantScopeField) {
      if (this.policy.requireTenantScope && !ctx.principal.tenantId) {
        throw new TenantScopeRequiredError(model, rowPolicy.tenantScopeField);
      }
      if (ctx.principal.tenantId) {
        filters.push({
          field: rowPolicy.tenantScopeField,
          op: 'eq',
    ...
    if (rowPolicy.ownershipScopeField && ctx.principal.userId) {
      filters.push({
        field: rowPolicy.ownershipScopeField,
        op: 'eq',
    ...
    // Soft delete filter
    if (rowPolicy.softDeleteField && !rowPolicy.includeSoftDeleted) {
      filters.push({
        field: rowPolicy.softDeleteField,
        op: 'is_null',
        value: true,
      });
    }
